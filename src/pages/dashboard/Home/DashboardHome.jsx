...
const DashboardHome = () => {
  ...
  const fetchReviewsCount = async () => {
    const response = await axios.get(
      `https://dream-craft-server.verce.app/review/${user?.email}`
    );
  ...
  const fetchBookingsCount = async () => {
    const response = await axios.get(
      `https://dream-craft-server.vecel.app/order/${user?.email}`
    );
    if (response.status === 200) {
  ...
  const fetchProductsOrderCount = async () => {
    const response = await axios.get(
      `https://dream-craft-server.vercel.app/poduct-orders/${user?.email}`
    );
    if (response.status === 200) {
